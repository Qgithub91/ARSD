...
node.attri<-read.csv(paste0(target.group, ".0.98.20250408.zp.result.tsv"),header = T, sep="\t")
module.hub<-as.character(node.attri$name[node.attri$z > 2.5 & node.attri$p <= 0.62])

module.hub.saving <- module.hub %>% 
  as.data.frame() %>% 
  left_join(taxonomy, by = c("." = "otus")) %>% 
  left_join(otu.taxa[,c("otus", "staxa")], by = c("." = "otus"))
write.table(module.hub.saving, paste0(target.group, ".", format(Sys.time(), "%Y%m%d"), ".module.hub.tsv"), sep = "\t", row.names = F, quote = F)
...
currentdat = dat1
write.table(currentdat, paste0(target.group, ".", format(Sys.time(), "%Y%m%d"), ".target_removal.tsv"), sep = "\t", row.names = F, quote = F)
